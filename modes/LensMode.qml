...
QtObject {
    ...
    function getCommand(x, y, w, h, openIn) {
        // [OPTIMIZATION] Optimized Lens build and open logic.
        // Uses more robust string building and ensuring cleanup happens even if some steps fail.
        ...
        if (openIn === "sidebar") {
            ...
            openCmd = `OLD_CLIP=$(wl-paste --no-newline --type text/plain 2>/dev/null || true); ` + `printf '%s' 'file://${lensHtml}' | wl-copy && ` + `sleep 0.1 && ` + `wlrctl window focus ${bc} && ` + `sleep 0.2 && ` + `wtype -M alt -k s -m alt && ` + `sleep 0.3 && ` + `if [ "$(wl-paste)" = "OPEN_SIDEBAR_PLEASE" ]; then ` + `wtype -M alt -k g -m alt && ` + `sleep 0.1 && ` + `printf '%s' 'file://${lensHtml}' | wl-copy ; ` + `fi; ` + `sleep 1; printf '%s' "$OLD_CLIP" | wl-copy`;
        }
        ...
        // [FIX] Ensure cleanup of temporary files after a delay
